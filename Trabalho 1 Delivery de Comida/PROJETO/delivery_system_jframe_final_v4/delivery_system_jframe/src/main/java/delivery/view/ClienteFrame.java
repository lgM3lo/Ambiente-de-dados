...
public class ClienteFrame extends JFrame {
    ...
    private boolean validarCampos() {
        ...
        // Validação de Telefone: deve conter apenas números, espaços, hífens, parênteses e o sinal de mais
        if (!telefone.matches("[0-9\\s\\-()\\+]+")) {
            showWarning("O campo Telefone contém caracteres inválidos. Use apenas números, espaços, hífens, parênteses ou o sinal de mais.");
            telefoneField.requestFocus();
            return false;
        }
        ...
        // Validação de Endereço: deve conter pelo menos uma letra (para evitar apenas números)
        if (!endereco.matches(".*[a-zA-Z].*")) {
            showWarning("O campo Endereço deve conter pelo menos uma letra (não pode ser apenas números).");
            enderecoField.requestFocus();
            return false;
        }
